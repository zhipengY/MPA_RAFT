...
from gma import Aggregate, Attention
...
class MPA(nn.Module):
    def __init__(self, channel, k_size=3):
        ...
        self.conv2_3 = nn.Conv2d(4 * channel, channel, kernel_size=1)
        ...
        self.conv_h = nn.Conv2d(channel, 1, kernel_size=(7, 1), stride=1, padding=(3, 0))
        self.conv_w = nn.Conv2d(channel, 1, kernel_size=(1, 7), stride=1, padding=(0, 3))
        self.convout = nn.Conv2d(2 * channel, channel, 1, 1, 0)
        self.sigmoid = nn.Sigmoid()
        self.norm = nn.BatchNorm2d(channel)
        self.relu = nn.SiLU()
        self.maxpool1 = nn.Conv2d(channel,channel,1)
    ...
    def forward(self, x):
        ...
        x_h = torch.sigmoid(self.conv_h(x))
        x_w = torch.sigmoid(self.conv_w(x))
        x_maxp = torch.sigmoid(self.maxpool(x))
        x_avgp = torch.sigmoid(self.avgpool(x))
        # x_hw = torch.cat((x_w,x_h),1)
        ...
        x_c3 = x_maxp * x
        x_c4 = x_avgp * x
        x_c = torch.cat((x_c1, x_c2, x_c3, x_c4),1)
        ...
        x = torch.cat((x * y.expand_as(x),  x_c), dim=1)
        x = self.convout(x)
...
if __name__ =="__main__":
    ...
    input2 = torch.randn(1, 128, 32, 32)
    flops, params = profile(mpa_, inputs=(input1,))
