...
class PIV(FlowDataset):
    def __init__(self, aug_params=None, split='training', root=''):
        ...
        for scene in os.listdir(root):
            #     imgs1 = sorted(glob(osp.join(root, scene + "/train_data/", '*img1.tif')))
            #     imgs2 = sorted(glob(osp.join(root, scene + "/train_data/", '*img2.tif')))
            #     flow_ = sorted(glob(osp.join(root, scene + "/train_data/", '*.flo')))

            imgs1 = sorted(glob(osp.join(root, scene + "/train_data/", '*img1.jpg')))
            imgs2 = sorted(glob(osp.join(root, scene + "/train_data/", '*img2.jpg')))
            flow_ = sorted(glob(osp.join(root, scene + "/train_data/", '*.npy')))
    
            # imgs1 = sorted(glob(osp.join(root, scene, '*image1.tif')))
            # imgs2 = sorted(glob(osp.join(root, scene, '*image2.tif')))
            # flow_ = sorted(glob(osp.join(root, scene, '*.npy')))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    # elif args.stage == 'PIV_MAT':
    #     aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
    #     train_dataset = PIV_MAT(aug_params, split='training')
